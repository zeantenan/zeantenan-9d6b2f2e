...
export const Route = createFileRoute("/produk/$slug")({
  ...
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: `Belanja ${params.slug.replace(/-/g, " ")} original dari Pekalongan. Kualitas premium, motif batik khas Kota Batik Indonesia.`,
      },
      { property: "og:title", content: `${params.slug.replace(/-/g, " ")} — ZEAN TENAN` },
      {
        property: "og:description",
        content: `${params.slug.replace(/-/g, " ")} original ZEAN TENAN — langsung dari pengrajin Pekalongan.`,
      },
    ],
  }),
  notFoundComponent: () => (
    ...
    <PublicLayout>
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <nav className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
          ...
          ·{" "}
          <a href="/produk" className="hover:text-foreground">
            Katalog
          </a>{" "}
        ...
        <div className="mt-8 grid gap-12 lg:grid-cols-2">
          <div>
            <div className="aspect-[4/5] w-full overflow-hidden border border-border bg-secondary">
              {images[activeImg] ? (
                <img
                  src={publicMediaUrl(images[activeImg].url)}
                  alt={product.name}
          ...
          <div>
            ...
            <div className="mt-8 flex items-center gap-3">
              <div className="inline-flex items-center border border-border">
                ...
                <button className="px-3 py-2" onClick={() => setQty(qty + 1)} aria-label="Tambah">
                  +
                </button>
              </div>
            </div>
