...
function AkunPage() {
  ...
  return (
    <AccountLayout title={`Halo, ${profile?.full_name || "Pelanggan"}`} description="Pantau aktivitas akun dan pesanan Anda.">
      <div className="grid gap-4 sm:grid-cols-3">
        <Stat label="Total Pesanan" value={String(orders.length)} />
        <Stat label="Sedang Diproses" value={String(orders.filter((o: any) => !["selesai","dibatalkan","ditolak"].includes(o.status)).length)} />
        <Stat label="Selesai" value={String(orders.filter((o: any) => o.status === "selesai").length)} />
      </div>

      <div className="mt-10">
        <div className="flex items-end justify-between border-b border-border pb-3">
          <h2 className="font-display text-xl">Pesanan Terbaru</h2>
          <Link to="/pesanan" className="text-xs text-primary hover:underline">Lihat semua</Link>
        </div>
        {recent.length === 0 ? (
          ...
          <ul className="divide-y divide-border">
            {recent.map((o: any) => (
              <li key={o.id}>
                <Link to="/pesanan/$orderNumber" params={{ orderNumber: o.order_number }} className="flex items-center justify-between gap-3 py-4">
                  <div>
