...
function ProfilPage() {
  ...
  const m = useMutation({
    mutationFn: updFn,
    onSuccess: () => { toast.success("Profil diperbarui"); qc.invalidateQueries({ queryKey: ["profile"] }); },
  });
  const dm = useMutation({
    mutationFn: delFn,
    onSuccess: async () => { await supabase.auth.signOut(); window.location.href = "/"; },
  });

  return (
    <AccountLayout title="Profil" description="Kelola informasi pribadi Anda.">
      <form onSubmit={form.handleSubmit((v) => m.mutate({ data: v }))} className="max-w-lg space-y-4">
        <div>
        ...
        <Button type="submit" disabled={m.isPending} className="rounded-none">Simpan Perubahan</Button>
      </form>

      <div className="mt-16 border-t border-border pt-8">
        <h3 className="font-display text-lg text-destructive">Hapus Akun</h3>
        <p className="mt-2 max-w-xl text-sm text-muted-foreground">
          Tindakan ini akan menghapus akun Anda secara permanen beserta seluruh data terkait. Tidak dapat dibatalkan.
        </p>
        <Button
          variant="outline"
          ...
          onClick={() => {
            if (confirm("Yakin ingin menghapus akun? Tindakan ini tidak bisa dibatalkan.")) dm.mutate({} as any);
          }}
        >
          Hapus Akun Permanen
        </Button>
